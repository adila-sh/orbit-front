...
import { Clock3Icon, MessageSquareIcon, PlusIcon, SendIcon, Trash2Icon } from "lucide-react";
...
import { AssigneeAvatar, PriorityBadge, StatusBadge, TypeBadge } from "@/features/issues/issue-meta";
import {
  createComment,
  createDependency,
  deleteDependency,
...
/**
 * Full issue detail body, shared between the `/issues/$issueId` route and the
 * board's issue dialog. When `hideHeader` is set the identifier + title block is
 * omitted (the dialog renders its own accessible header instead).
 */
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      {target && <span className="ml-2 text-muted-foreground">{target.title}</span>}
                    </Link>
                    <Button
                      variant="ghost"
                      size="icon-sm"
